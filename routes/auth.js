...
router.post("/login", asyncHandler(async (req,res) => {
    ...
    if(user && (await user.matchPassword(password))) {
        ...
        res.cookie("token", generateToken(responseData._id), { secure: true, sameSite: "Lax"}) //send the user id on token
...
router.post("/register", asyncHandler( async (req,res) => {
    ...
    if(user){  
        res.cookie("token", generateToken(user._id), { secure: true, sameSite: "Lax"}) //send the user id on token
        res.status(201).send({
            _id: user._id,
            name: user.name,
            email: user.email,
...
router.get("/logout", asyncHandler( async (req,res) => {
    res.cookie("token", 'none', { secure: true, sameSite: "Lax", expires: new Date(Date.now() + 2 * 1000),})
    res
        .status(201)
